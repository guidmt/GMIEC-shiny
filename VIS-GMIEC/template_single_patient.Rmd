...
sample_name<-params$name_patient
  ...
  link_to_string<-paste(paste("(",paste(paste(paste(getwd(),"output_vis/",sep="/"),paste("report.",sample_name,".single.patient",sep=""),sep=""),".html",sep=""),sep=""),")",sep="")
  
  LinkPatient<-paste(string,link_to_string,sep="")

  
idx_number_drugs<-grep(tab_patient[,1],pattern="drugs_module")
idx_number_genes<-grep(tab_patient[,1],pattern="genes_module")

idx_number_sad<-grep(tab_patient[,1],pattern="sad")
idx_number_sam<-grep(tab_patient[,1],pattern="score_alteration_module")
idx_number_sadrugs<-grep(tab_patient[,1],pattern="score_alteration_drugs")
